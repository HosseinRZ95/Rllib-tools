...
class Postprocessing:
    ...
    """Based on Rllib postprocessing.
    you can now use your own credit_assignment logic in compute advantage as example"""
    "this pp has a reward for holding and discount for trade pos"
...
def adjust_nstep(n_step: int, gamma: float, batch: SampleBatch) -> None:
    ...
    print('batch[SampleBatch.REWARDS]',batch[SampleBatch.REWARDS])
@DeveloperAPI
def compute_advantages(
    ...
    if use_gae:
        ...
        # print('vpred_t is :',vpred_t)
        delta_t = rollout[SampleBatch.REWARDS] + gamma * vpred_t[1:] - vpred_t[:-1]
        print('rollout[SampleBatch.REWARDS] is:', rollout[SampleBatch.REWARDS])
        ...
        nonzero = np.nonzero(rollout[SampleBatch.REWARDS])[0]
        # print('reward collected',rollout[SampleBatch.REWARDS])
        p=0
        # print('info',rollout[SampleBatch.INFOS])

        for i in nonzero:
            rollout[SampleBatch.REWARDS][rollout[SampleBatch.INFOS][i]['s']] = rollout[SampleBatch.REWARDS][i] 
            rollout[SampleBatch.REWARDS][i] = 0
        # print('rollout[SampleBatch.newREWARDS] iss:', rollout[SampleBatch.REWARDS])
        

        # print('rewards_plus_v is:', rewards_plus_v)
        discounted_returns = rollout[SampleBatch.REWARDS]
...
def discount_cumsum(x: np.ndarray, gamma: float) -> np.ndarray:
    ...
    Examples:
        ...
        ...        3.0])
    """
    discountcumsum = scipy.signal.lfilter([1], [1, float(-gamma)], x[::-1], axis=0)[::-1]
    # print('discountcumsum is :',discountcumsum)
    return discountcumsum
